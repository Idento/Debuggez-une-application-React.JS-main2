...
const Page = () => {
  const {data} = useData()
  const lastOne = data ? [...data.events].sort((evtA, evtB)=>
  ...
  return <>
    ...
    <footer className="row">
      <div className="col presta">
        ...
        {lastOne ? <EventCard
          imageSrc={lastOne?.at(-1)?.cover}
          title={`${lastOne?.at(-1)?.title}`}
          date={new Date(lastOne?.at(-1)?.date)}
          small
          label={`${lastOne?.at(-1)?.type}`}
        />: null}
      </div>
